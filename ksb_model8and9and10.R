...
# Create segment_id (model 5)(age)
# input$segment_id <- 
#   ifelse( input$C6 < 30, 1, 
#   ifelse( input$C6 >= 45, 3, 
#           2 ))
# Create segment_id (model 6)(gender)
# input$segment_id <- 
#   ifelse( input$C5 == "H", 1, 
#           ifelse( input$C5 == "V", 3, 
#                   2 ))
# Create segment_id (model 7)(renta - gross income)
input$segment_id <-
  ifelse( as.numeric(input$C23) < 60000, 1,
          ifelse( as.numeric(input$C23) >= 200000, 3,
                  2 ))
...
df2 <- df[,c(1,50)]
write.csv(df2, file = "../ksb_model7_solution_temp.csv", row.names = FALSE, quote = FALSE) 
